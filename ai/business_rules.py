...
def apply_business_rules(fields: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Only normalize brand field itself without fabricating unextracted declarations
    if "brand" in fields and isinstance(fields["brand"], dict):
        fields["brand"]["value"] = canonical_name
    elif "brand" not in fields:
        fields["brand"] = _build_override_field("brand", {"value": canonical_name}, canonical_name)
